...
const createMenu = (item) => {
  ...
  return {
    ...
    payment: () => {
      const foodPrice = menu.reduce(function (sum, it) {
        return item.food[it] ? sum + item.food[it] : sum;
      }, 0);
      const drinkPrice = menu.reduce(function (sum, it) {
        return item.drink[it] ? sum + item.drink[it] : sum;
      }, 0);
